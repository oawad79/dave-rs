use macroquad::{audio::play_sound_once, math::{vec2, Rect, Vec2}};
...
use super::{collectibles::CollectibleType, Game, GameState};
...
impl CollisionManager {
    ...
    pub fn check_warp_zone_collision(
    ...
    pub fn check_door_collision(
        ...
        if score_board.game_won && Player::overlaps(player_pos, &Rect::new(
            door.world_x,
            door.world_y - 32.0,
            32.0,
            32.0,
    ...
    pub fn check_special_item_collisions(
        ...
        if let Some(j) = jetpack { 
            if !player.has_jetpack && Player::overlaps(player_pos, &Rect::new(
                j.world_x,
                j.world_y - 32.0,
                32.0,
                32.0,
        ...
        if let Some(g) = gun { 
            if !player.has_gun && Player::overlaps(player_pos, &Rect::new(
                g.world_x,
                g.world_y - 32.0,
                32.0,
                32.0,
            ...
            if monster.alive {
                ...
                if Player::overlaps(player_pos, &monster.monster_rectangle()) {
                    ...
                    if explosions.is_empty() {
                        explosions.push((Emitter::new(EmitterConfig {
                            amount: 40,
